        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            <div class="lg:col-span-1">
                <div class="sticky top-6 space-y-6">
                    <div  id="changes-section" class="bg-white dark:bg-slate-800 rounded-2xl shadow-sm border border-gray-200 dark:border-slate-700 p-6"
                        x-data="{ open: false }">
                        ...
                        <div x-show="open" x-transition.opacity.duration.200ms>
                            <!-- Add Change Form -->
                            <form id="changes-form" action="{{ route('crm-projects.activities.store', $project->id) }}" method="POST" enctype="multipart/form-data"
                                ...
                                <div class="mb-3" x-data="{ fileName: '' }">
                                    ...
                                    <template x-if="fileName">
                                        <div class="mt-1.5 flex items-center justify-between gap-2 px-2.5 py-1 bg-indigo-900/40 border border-indigo-700/50 rounded-md text-xs text-indigo-300">
                                            <div class="flex items-center gap-1.5 truncate">
                                                <svg class="w-3.5 h-3.5 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15.172 7l-6.586 6.586a2 2 0 102.828 2.828l6.414-6.586a4 4 0 00-5.656-5.656l-6.415 6.585a6 6 0 108.486 8.486L20.5 13"></path></svg>
                                                <span class="truncate" x-text="fileName"></span>
                                            </div>
                                            <button type="button" @click="$refs.changeAttachment.value = ''; fileName = ''" class="text-indigo-400 hover:text-white font-bold ml-2">&times;</button>
                                        </div>
                                    </template>
                                </div>
                                    ...
                                    @foreach($project->activities as $activity)
                                        <div
                                            ...
                                            @if($activity->attachment_path)
                                                <div class="mb-2">
                                                    <a href="{{ route('api.activities.attachment.show', $activity->id) }}" target="_blank" class="inline-flex items-center gap-1.5 text-xs text-indigo-600 dark:text-indigo-400 hover:underline bg-indigo-50 dark:bg-indigo-900/30 px-2 py-1 rounded border border-indigo-100 dark:border-indigo-800/40">
                                                        <svg class="w-3.5 h-3.5 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15.172 7l-6.586 6.586a2 2 0 102.828 2.828l6.414-6.586a4 4 0 00-5.656-5.656l-6.415 6.585a6 6 0 108.486 8.486L20.5 13"></path></svg>
                                                        <span class="truncate max-w-[200px]">{{ $activity->attachment_name ?? 'Attachment' }}</span>
                                                    </a>
                                                </div>
                                            @endif
                                            ...
                                            <div class="flex justify-between items-center">
                                                <div class="flex-1 min-w-0" x-data="{ date: new Date('{{ $activity->created_at->toISOString() }}') }">
                                                    <span class="text-xs text-gray-500 dark:text-gray-400" x-text="date.toLocaleString(undefined, { month: 'numeric', day: 'numeric', year: 'numeric', hour: 'numeric', minute: '2-digit', second: '2-digit', hour12: true })">{{ $activity->created_at->format('n/j/Y, g:i:s A') }}</span>
                                                </div>
                    ...
                    <!-- Enhancements Box -->
                    <div id="enhancements-section" class="bg-white dark:bg-slate-800 rounded-2xl shadow-sm border border-gray-200 dark:border-slate-700 p-6"
                        x-data="{ open: false }">
                        <button @click="open = !open" type="button"
                            class="w-full flex items-center justify-between mb-4 focus:outline-none group">
                            <div class="flex items-center gap-2">
                        ...
                        <div x-show="open" x-transition.opacity.duration.200ms>
                            ...
                            <form id="enhancements-form" action="{{ route('crm-projects.enhancements.store', $project->id) }}" method="POST" enctype="multipart/form-data"
                                class="mb-6 pb-6 border-b border-gray-100 dark:border-slate-700"
                                style="padding-bottom: 1.5rem;">
                                @csrf
                                <div class="mb-3" x-data="{ fileName: '' }">
                                    ...
                                    <template x-if="fileName">
                                        <div class="mt-1.5 flex items-center justify-between gap-2 px-2.5 py-1 bg-indigo-900/40 border border-indigo-700/50 rounded-md text-xs text-indigo-300">
                                            ...
                                            <button type="button" @click="$refs.enhancementAttachment.value = ''; fileName = ''" class="text-indigo-400 hover:text-white font-bold ml-2">&times;</button>
                                        </div>
                                    </template>
                                        ...
                                        <div
                                            ...
                                            @if($enhancement->time_estimate)
                                                <div
                                                    ...
                                                    {{ trim(preg_replace('/(hrs?|hours?)$/i', '', $enhancement->time_estimate)) }} hrs
                                                </div>
                                            @else
                                                <div class="mb-2"></div>
                                            @endif
                                            <div class="flex justify-between items-center">
                                                <div class="flex-1 min-w-0" x-data="{ date: new Date('{{ $enhancement->created_at->toISOString() }}') }">
                                                    <span class="text-xs text-gray-500 dark:text-gray-400" x-text="date.toLocaleString(undefined, { month: 'numeric', day: 'numeric', year: 'numeric', hour: 'numeric', minute: '2-digit', second: '2-digit', hour12: true })">{{ $enhancement->created_at->format('n/j/Y, g:i:s A') }}</span>
                                                </div>
                                                <div class="flex items-center gap-1.5">
                                                    <div
                                                        class="w-4 h-4 rounded-full bg-indigo-100 dark:bg-indigo-900 text-indigo-700 dark:text-indigo-400 flex items-center justify-center text-[8px] font-bold">
